...
public class GitHub {
    ...
    public static Release getLatestRelease(ResourceBundle properties) {
        try {
            ...
            Asset[] assets = new Gson().fromJson(latestRelease.getAssets(), Asset[].class);
            for (Asset asset : assets) {
                if (asset.getName().endsWith(".exe")) {
                    latestRelease.setFileName(asset.getName());
                    latestRelease.setFileUpdatedAt(asset.getUpdated_at());
                    latestRelease.setFileDownloadURL(asset.getBrowser_download_url());
                    break;
                }
            }
